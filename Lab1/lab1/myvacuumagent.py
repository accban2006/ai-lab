...
class MyVacuumAgent(Agent):
    ...
    def get_bfs_path(self):
        """ BFS algorithm to find the shortest path to the nearest UNKNOWN tile and return home if all already explored """
        # Goal 1: Find the nearest unexplored (UNKNOWN) tile
        queue = [(self.state.pos_x, self.state.pos_y, [])]
        ...
        while queue:
            ...
            for dx, dy in [(0, -1), (1, 0), (0, 1), (-1, 0)]:
                ...
                if 0 <= nx < self.state.world_width and 0 <= ny < self.state.world_height:
                    # Do not walk through known walls
                    if (nx, ny) not in visited and self.state.world[nx][ny] != AGENT_STATE_WALL:
                        visited.add((nx, ny))
                        queue.append((nx, ny, path + [(nx, ny)]))

        # Goal 2: Return to HOME (1,1) if the whole map is explored
        queue = [(self.state.pos_x, self.state.pos_y, [])]
        visited = set([(self.state.pos_x, self.state.pos_y)])
